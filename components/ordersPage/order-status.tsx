  ...
  return (
    <DropdownMenu>
      <DropdownMenuTrigger className="flex flex-row items-center gap-x-2 focus:outline-none">
        <p className={`${nunitoSans.className} text-[14px] font-semibold`}> Orders Status</p>
        <Image src={'/dropdown.svg'} width={20} height={24} alt='arrow-down' />
      </DropdownMenuTrigger>
      
      <DropdownMenuContent className="w-[420px] h-[250px] py-1 bg-white rounded-lg shadow-md flex flex-col gap-y-1">
          ...
          {buttons.map((label, index) => (
            <Button
              ...
              className={`w-[120px] h-[30px] rounded-[17px] shadow-none border hover:bg-white border-[#979797] ${
                selectedButtons.includes(label) ? 'bg-[#4880FF] text-white' : 'bg-white text-black'
              }`}
            >
